...
待办(真实使用 / 发布):
1. 按 [docs/SETUP.md](docs/SETUP.md) 接入:最省事是 popover ⋯ →「接入 agent…」复制提示词、粘进 Claude Code 让它自己把 hooks 幂等合并进 `~/.claude/settings.json`(推荐内建 `/claude/hooks` HTTP hook,零依赖;端口黏住,写一次长期有效);跑真实 Claude 会话端到端验证计数与断言翻转。
...
Sources/BusyElf/
  ...
  Server/      LoopbackServer(NWListener loopback + 手写 HTTP 解析 + 端口黏住)、Router(路径分流)、TaskEvent(中立)、ClaudeHookEvent(内建 Claude 适配 + 接入提示词)、GenericSetupPrompt(中立接入提示词)
