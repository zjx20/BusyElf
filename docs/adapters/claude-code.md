...
- **"当前工作 / 当前回复"展示**:加 `PreToolUse`(工具开始即显示)、`PostToolUse`(工具做完打 ✓ + 权限授权后复活)、`PostToolUseFailure`(工具失败打 ✗,常态非中断)和 `MessageDisplay`(实时回复)。
...
URL 里的端口写死成默认 `17872`。**端口是"黏住(sticky)"的**:BusyElf 首启探测出一个可用端口(17872 被占就往后 17873/74/75,实在没有就让系统分配)并**固定下来**,之后每次只绑这个端口——所以 URL 写一次长期有效。若这个固定端口某次被别的进程占了,BusyElf **不会漂移到别的端口**,而是在 UI 报错(菜单栏红角标 + 面板顶部横幅 [重试]/[改端口]);释放端口点重试即可。固定端口在右键菜单顶部 / 面板底部显示。想换端口:面板「更多设置」改,或 `defaults write elf.busyelf httpPort 12345` 后重开——**换端口后所有 hook URL 要相应更新**(最省事是用 popover ⋯ →「接入 agent…」重新复制提示词)。方式 B 的 curl URL 同理。
...
```json
{
  "hooks": {
    ...
    "PostToolUse": [{ "hooks": [{ "type": "command",
      "command": "jq -c '{id:(.session_id+(if .agent_id then \"#\"+.agent_id else \"\" end)), parentId:(if .agent_id then .session_id else null end), name:.agent_type, tool:.tool_name, toolInput:(.tool_input.command // .tool_input.file_path // \"\"), toolComplete:true}' | curl -sS -m2 -X POST http://127.0.0.1:17872/v1/task/update --data-binary @- >/dev/null 2>&1 || true" }] }],

    "PostToolUseFailure": [{ "hooks": [{ "type": "command",
      "command": "jq -c '{id:(.session_id+(if .agent_id then \"#\"+.agent_id else \"\" end)), parentId:(if .agent_id then .session_id else null end), name:.agent_type, tool:.tool_name, toolInput:(.tool_input.command // .tool_input.file_path // \"\"), toolFailed:true, toolError:.error}' | curl -sS -m2 -X POST http://127.0.0.1:17872/v1/task/update --data-binary @- >/dev/null 2>&1 || true" }] }],
...
| `tool_name` / `tool_input.*` | `PostToolUse` / `PostToolUseFailure` | → `tool` / `toolInput`(按工具形状取第一个非空) |
| `error` | `PostToolUseFailure` | → `toolError`(工具失败原因,作动作行 tooltip);并令 `toolFailed=true` 打 ✗ |
