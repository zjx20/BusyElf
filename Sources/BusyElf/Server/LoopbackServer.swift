...
///
/// - 仅 loopback 可达靠 `NWParameters.requiredInterfaceType = .loopback`(免本地网络隐私弹窗,
///   而非靠 bind 127.0.0.1)。
/// - 只手解析两个事:请求行(METHOD PATH)+ `Content-Length` 决定 body 长度。够用即止。
/// - 端口被占用时按候选表回退;实际监听端口暴露在 `port`,供 UI/诊断显示。
final class LoopbackServer {
    ...
    private let candidatePorts: [UInt16] = [17872, 17873, 17874, 17875]
    ...
    private func startListener(candidateIndex index: Int) {
        ...
        params.requiredInterfaceType = .loopback        // 仅回环
        ...
        listener.stateUpdateHandler = { [weak self] state in
            ...
            case .ready:
                ...
                NSLog("BusyElf: 监听 127.0.0.1:\(portNumber)")
